...
class ClusteringMetrics:
    ...
    def silhouette_score(self, embeddings: List, cluster_labels: List) -> float:
        ...
        if len(np.unique(cluster_labels)) > 1:
            x = silhouette_score(embeddings, cluster_labels)
        else:
    ...
    def purity_score(self, true_labels: List, cluster_labels: List) -> float:
        ...
        column_max_sum = np.sum(np.max(contingency_matrix, axis=0))

        total_sum = np.sum(contingency_matrix)

        return column_max_sum / total_sum
